...
Module.register("v2-Octo",{
	...
	getElecExpData: function(retries) {
		Log.log("getElecExpData(retries=" + retries + ")");
		...
		{
			var elecExpDataRequest = new XMLHttpRequest();
			elecExpDataRequest.open("GET", this.config.elecExpApiUrl, true);
			elecExpDataRequest.setRequestHeader("Authorization","Basic " + hash);
			elecExpDataRequest.onreadystatechange = function() {
				Log.log("getElecExpData() readyState=" + this.readyState);
				if (this.readyState === 4) {
					Log.log("getElecExpData() status=" + this.status);
					if (this.status === 200) {
						self.processElecExpData(JSON.parse(this.response));
						retries=0;
					} else if (this.status === 401) {
						self.elecExpLoaded = false;
						self.updateDom(self.config.animationSpeed);
						Log.error(self.name, "getElecExpData() 401 error. status=" + this.status);
					} else {
						self.elecExpLoaded = false;
						self.updateDom(self.config.animationSpeed);
						Log.error(self.name, "getElecExpData() Could not load data. status=" + this.status);
					}
					
					if (retries>0) {
						retries=retries-1;
						self.scheduleElecExpRetry(retries);
					}
				}
			};
			elecExpDataRequest.send();
		}
	...
	scheduleElecExpRetry: function(retries) {
		Log.log("scheduleElecExpRetry() retries=" + retries);
		var self = this;
		setTimeout(function() {
			self.getElecExpData(retries);
		}, self.config.retryDelay);
	},
...
/*
		headerdatelabel.innerHTML = "<span class=\"fa fa-calendar-alt small\"></span> Date";
*/
		...
		headereleclabel.innerHTML = "<span class=\"fa fa-plug small\"></span> Electricity";
		headereleclabel.className = "small";
		headereleclabel.style.verticalAlign = "top";
		headereleclabel.style.textAlign = "center";

		var headerelecexplabel = document.createElement("td");
		headerelecexplabel.innerHTML = "<span class=\"fa fa-plug small\"></span> Export";
		headerelecexplabel.className = "small";
		headerelecexplabel.style.verticalAlign = "top";
		headerelecexplabel.style.textAlign = "center";
		...
		if(this.elecExpDataRequest) headerrow.appendChild(headerelecexplabel);
		...
		if(true)
		{
			...
			var elecexpdate;
			if (this.elecExpDataRequest) {
				if(typeof this.elecExpDataRequest.results[0] !== 'undefined') {
					elecexpdate = new Date(this.elecExpDataRequest.results[0].interval_start);
				}
			}
			...
			if(typeof elecexpdate == 'undefined') {
				elecexpdate = new Date();
			}
		...
		for(intLoop=0;intLoop<intDays;intLoop++)
		{
			...
			var thiselecexplabel = document.createElement("td");
			thiselecexplabel.innerHTML = "---";
			thiselecexplabel.className = "small";
			thiselecexplabel.style.textAlign = "center";
				...
				for(i=0;i<intDays;i++) {
					if(typeof this.elecDataRequest.results[i] !== 'undefined') {
						...
						if(edate.toLocaleDateString() == dteLoop.toLocaleDateString()) {
							...
							var strCol = "color:white";//could be green
			...
			if (this.elecExpDataRequest) {
				for(i=0;i<intDays;i++) {
					if(typeof this.elecExpDataRequest.results[i] !== 'undefined') {
						var edate = new Date(this.elecExpDataRequest.results[i].interval_start);
						if(edate.toLocaleDateString() == dteLoop.toLocaleDateString()) {

							var strCol = "color:white";//could be green
							var intVal = this.elecExpDataRequest.results[i].consumption.toFixed(this.config.decimalPlaces);
							if(intVal>=this.config.elecExpMedium)strCol="color:orange";
							if(intVal>=this.config.elecExpHigh)strCol="color:green";    //green  to indicate high export

							strUse = intVal + " kWh";
							strCost="";
							if(this.config.elecExpCostKWH>0)
								strCost = "£" + (Math.round(((intVal * this.config.elecExpCostKWH)+this.config.elecExpCostSC) * 100)/100).toFixed(2);

							//display electricity energy usage and cost here
							thiselecexplabel.innerHTML = "&nbsp;&nbsp;<span style=\"" + strCol + "\">" + strUse + " " + strCost + "</span>";
							thiselecexplabel.style.textAlign = "right";
			...
			if(this.elecExpDataRequest) thisrow.appendChild(thiselecexplabel);
	...
	processElecExpData: function(data) {
		Log.log("processElecExpData()");
		var self = this;
